...
const PoliceRegisterPage = () => {
  ...
  const [email, setEmail] = useState(""); // email as username
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:8080/api/police/register", {
        name: stationName,
        officerName,
        phone,
        username: email, // email used as login
        password,
        location: {
          type: "Point",
      ...
      // Save the station ID for dashboard/socket use
